...
void Direct3D::Release()
{
	for (int i = 0; i < SHADER_MAX; i++)
	{
		...
		SAFE_DELETE(shaderBundle[i].pRasterizerState_);
		SAFE_RELEASE(shaderBundle[i].pVertexLayout_);
		SAFE_DELETE(shaderBundle[i].pVertexLayout_);
		SAFE_RELEASE(shaderBundle[i].pPixelShader_);
		SAFE_DELETE(shaderBundle[i].pPixelShader_);
		SAFE_RELEASE(shaderBundle[i].pVertexShader_);
		SAFE_DELETE(shaderBundle[i].pVertexShader_);
	}

	SAFE_RELEASE(pDepthStencilView);
	SAFE_DELETE(pDepthStencilView);
